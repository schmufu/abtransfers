	...
	QObject(parent)
{
	...
	if (!dataStorage.exists()) {
        ...
        this->m_settings = new QSettings(iniFilename, QSettings::IniFormat, this);
        this->m_settings->setIniCodec(QTextCodec::codecForName("UTF-8"));
	...
	defValue = QDir::toNativeSeparators(defValue);
        this->m_dataDir = this->m_settings->value("Main/DataDir", defValue).toString();

	//file for known recipients
	defValue = homePath + "/.abtransfers/recipients.txt";
	defValue = QDir::toNativeSeparators(defValue);
	this->m_recipientsFilename =
                this->m_settings->value("Main/RecipientsFilename", defValue).toString();
	...
	this->m_accountdataFilename =
                this->m_settings->value("Main/AccountDataFilename", defValue).toString();

	//file for the history
	defValue = homePath + "/.abtransfers/history.ctx";
	defValue = QDir::toNativeSeparators(defValue);
	this->m_historyFilename =
                this->m_settings->value("Main/HistoryFilename", defValue).toString();
...
void abt_settings::loadTextKeyDescriptions()
{
	...
	this->m_textKeyDescr->clear();
        ...
        if (!this->m_settings->childGroups().contains("TextKeyDescriptions")) {
		//TextKexDescriptions doesnt exists, use default values
                this->m_settings->beginGroup("TextKeyDescriptions");
                this->m_settings->setValue("04", "Lastschrift (Abbuchungsauftragsverfahren)");
                this->m_settings->setValue("05", "Lastschrift (Einzugsermächtigungsverfahren)");
                this->m_settings->setValue("51", "Überweisung");
                this->m_settings->setValue("52", "Dauerauftrags-Überweisung");
                this->m_settings->setValue("53", "Lohn-, Gehalts-, Renten-Überweisung");
                this->m_settings->setValue("54", "Vermögenswirksame Leistung (VL)");
                this->m_settings->setValue("56", "Überweisung öffentlicher Kassen");
                this->m_settings->setValue("67", "Überweisung mit prüfziffergesicherten Zuordnungsdaten (BZÜ)");
                this->m_settings->setValue("69", "Spendenüberweisung");
                this->m_settings->endGroup();
	}
	
        this->m_settings->beginGroup("TextKeyDescriptions");
	//go through all keys and store the values in a QHash
        foreach (QString key, this->m_settings->allKeys()) {
                QString text = this->m_settings->value(key, tr("Unbekannt")).toString();
...
void abt_settings::saveWindowStateGeometry(QByteArray state,
					   QByteArray geometry)
{
        this->m_settings->setValue("Main/WindowState", state);
        this->m_settings->setValue("Main/WindowGeometry", geometry);
}

QByteArray abt_settings::loadWindowState() const
{
        return this->m_settings->value("Main/WindowState", QVariant()).toByteArray();
}

QByteArray abt_settings::loadWindowGeometry() const
{
        return this->m_settings->value("Main/WindowGeometry", QVariant()).toByteArray();
}
...
void abt_settings::saveSelAccountInWidget(const QString &widgetName, const aqb_AccountInfo *acc)
{
	...
	groupname.append(widgetName);
        this->m_settings->setValue(groupname, acc->get_ID());
}
...
int abt_settings::loadSelAccountInWidget(const QString &widgetName) const
{
	QString groupname("Main/Widget");
	groupname.append(widgetName);
        return this->m_settings->value(groupname, -1).toInt();
}
...
{
        return this->m_settings->value(QString("Dialogs/Show").append(dialogType),
				     true).toBool();
}

void abt_settings::setShowDialog(const QString &dialogType, bool show)
{
        this->m_settings->setValue(QString("Dialogs/Show").append(dialogType), show);
}
...
{
        return this->m_settings->value(QString("LoadAtStart/").append(jobname),
				     false).toBool();
}

void abt_settings::setAppendJobToOutbox(const QString &jobname, bool get)
{
        this->m_settings->setValue(QString("LoadAtStart/").append(jobname), get);
}
...
{
        return this->m_settings->value("Options/autoAddNewRecipients", true).toBool();
}

//public
void abt_settings::setAutoAddNewRecipients(bool value)
{
        this->m_settings->setValue("Options/autoAddNewRecipients", value);
}
...
{
        this->m_settings->beginGroup("ImExportFavorites");
	QStringList retList;

        foreach(const QString group, this->m_settings->childGroups()) {
                this->m_settings->beginGroup(group);

                foreach(const QString key, this->m_settings->childKeys()) {
...
bool abt_settings::isProfileFavorit(const QString &name) const
{
	QString key = QString("ImExportFavorites/").append(name);
        return this->m_settings->value(key, false).toBool();
}

void abt_settings::setProfileFavorit(const QString &name, bool favorit)
{
	QString key = QString("ImExportFavorites/").append(name);
        this->m_settings->setValue(key, favorit);
}
...
{
        this->m_settings->beginGroup("Options/Advanced/");
        this->m_settings->remove(option);
        this->m_settings->endGroup();
}
...
void abt_settings::saveColWidth(const QString &name, int col, int width)
{
	...
	key.append(key2);
        this->m_settings->setValue(key, width);
}
...
int abt_settings::getColWidth(const QString &name, int col, int def)
{
	...
	key.append(key2);
	bool convOK;
        int ret = this->m_settings->value(key, def).toInt(&convOK);
	if (convOK) {
		return ret;
	}
...
void abt_settings::setRecipientsFilename(const QString &filename)
{
	if (filename.isEmpty()) {
        ...
        this->m_settings->setValue("Main/RecipientsFilename", filename);
...
void abt_settings::setAccountDataFilename(const QString &filename)
{
	if (filename.isEmpty()) {
        ...
        this->m_settings->setValue("Main/AccountDataFilename", filename);
...
void abt_settings::setHistoryFilename(const QString &filename)
{
	if (filename.isEmpty()) {
        ...
        this->m_settings->setValue("Main/HistoryFilename", filename);
...
void abt_settings::setDataDir(const QString &dirname)
{
	if (dirname.isEmpty()) {
        ...
        this->m_settings->setValue("Main/DataDir", dirname);
